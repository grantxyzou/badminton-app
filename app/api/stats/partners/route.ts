...
// Session ids sort by date within a group (`session-YYYY-MM-DD`, prefixed for
// any group but BPM), so a cutoff is just the id a session on that date would
// have — built per group, because a prefixed id sorts before a bare one.
function cutoffSessionId(weeks: number, groupId: string): string {
  const d = new Date();
  d.setDate(d.getDate() - weeks * 7);
  return sessionIdFromDate(d.toISOString(), groupId);
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const cutoff = cutoffSessionId(weeks, scope.groupId);
    ...
    const resources = await scope.query<{ sessionId?: unknown; name?: unknown; removed?: unknown }>('players', {
