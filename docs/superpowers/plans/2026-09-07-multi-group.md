...
- [ ] 1a the session family: `sessions`, `players`, `announcements`, `skills`, `gameResults`; `lib/announcements.ts`, `lib/kudosEligibility.ts` (`sessionPrefix`); six queries bind `@sid` instead of `@sessionId` (the mock silently applies no filter) — rename them
- [ ] 1a **the pointer doc is a session row to every list query** (Phase 1a-i review): a non-BPM group's `${g}:active-session-pointer` doc is stamped with its group and carries no `datetime`, and the ten session-list exclusions match only the bare `POINTER_ID` / `'current-session'`. The accessor's `sessions` reads must exclude `groupDocId(groupId, POINTER_ID)` (or `NOT IS_DEFINED(c.activeSessionId)`), the mock's `@pointerId` filter must honour the prefixed id, and a two-group test must prove a pointer never comes back as a session
