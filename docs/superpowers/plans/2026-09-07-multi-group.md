...
- [ ] 1b the rest (the 18 files left on `MIGRATION_BACKLOG`): `birds` (`lib/birdWrite.ts`, both birds routes, `session/bird-usage`), `aliases` (`lib/playerIdentity.ts`, the aliases route, member history), `kudos` (the kudos route's container), `stringingJobs` + `clubSettings` (`shopDocId` etc. in `lib/stringingShop.ts`, `lib/stringingStrings.ts`, `lib/stringingPricing.ts`; the seven stringing routes; `players/unpaid`'s charges read), `events` (`lib/events.ts`, `slice0`)
- [ ] 1b `stats/club/bands`, `stats/club/gear` and the level fold's self-seeds narrowed via `rosterMemberIds()` (needs `memberships` — may slide to Phase 2)
