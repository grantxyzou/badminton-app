...
- **`TOLERATE_UNSTAMPED`** (same file): while `true`, a row with no `groupId` reads as BPM's. The mock store's `@groupId` filter (`__tests__/mock-store-groupid.test.ts`) reads the SAME constant, so mock and Cosmos flip together — an isolation test written before the mock knew the name would have passed vacuously (unrecognised parameter = no filter).
