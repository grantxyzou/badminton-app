...
- **Fonts**: Space Grotesk (display — h1/h2/h3, wordmark, splash) + IBM Plex Sans (body — `var(--font-sans)`) + JetBrains Mono (data — `var(--font-mono)`). Self-hosted subset **WOFF2** files in `app/fonts/`, all three loaded via `next/font/local`. Italic variant wired for IBM Plex. Regenerate with `docs/subset-fonts.md`.
  - **This line was FALSE for JetBrains Mono until 2026-09-07** — it said all three were local while `app/layout.tsx` imported that one from `next/font/google` and `app/fonts/` held only two files. The cost was not cosmetic: `next/font/google` fetches at BUILD time, so `fonts.googleapis.com` being unreachable fails `next build` outright (`Error: next/font: Failed to fetch JetBrains Mono from Google Fonts`) and therefore fails a production deploy. It bought nothing — that loader downloads at build and serves from our own origin regardless, which is what `next/font/local` does without the round trip. Found only because the build was run with no network. **Never add a `next/font/google` import**; `__tests__/design-canary.test.ts` fails the build on one.
...
- **Automation hooks** (`.claude/settings.json` + `scripts/`): SEVEN now, each tied to a bug class this repo actually hit.
  ...
  - `check-plan-reviews.mjs` (SessionStart) — reports plan docs past a `**Review on:** YYYY-MM-DD` header. The flag-retirement mechanism generalised: `docs/plans/value-hub-slice-0.md` carried an honest kill criterion and went unread for ~9 weeks, by which time three of the four tracks it gated had shipped anyway. A criterion with no scheduled read is a note, not a gate. A plan with NO review line is fine — most are finished records — so the field stays meaningful rather than ceremonial. Reports, never blocks.
