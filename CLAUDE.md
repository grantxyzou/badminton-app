...
- **Auth taxonomy** (since 2026-04-30): three distinct concepts.
  ...
  - **Sign in** = name + PIN auth. Profile inline form OR HomeTab "Already a player?" link → RecoverySheet. POST `/api/players/recover` verifies against `members.pinHash` and returns **identity-only** (`deleteToken: null`) when no session player exists — sign-in does NOT auto-register the user for the active session. The user must tap Sign-up explicitly to join (commit 6046755, 2026-05-07). Constant-time miss against `FAKE_HASH`. Rate-limited 5/hr per `(name, IP)`.
