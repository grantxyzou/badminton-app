...
export default function AnomalyFeed({ refreshKey = 0 }: AnomalyFeedProps) {
  ...
  async function dismiss(code: string) {
    ...
    // Read current dismissed list, append, PATCH back.
    try {
      const sessionRes = await fetch(`${BASE}/api/session`, { cache: 'no-store' });
      const session = sessionRes.ok ? await sessionRes.json() : {};
      const current: string[] = Array.isArray(session.anomaliesDismissed) ? session.anomaliesDismissed : [];
      const next = Array.from(new Set([...current, code]));

      const res = await fetch(`${BASE}/api/session`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...session, anomaliesDismissed: next }),
      });
      if (res.ok) {
