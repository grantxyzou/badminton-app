...
import type { GearPick, GearPickCardStatus, ParkReason, PickReasonKey } from './GearPickCard';
...
export const ORDER: EquipmentCategory[] = ['racket', 'shoe', 'string', 'shuttle'];
...
export const SOURCED: EquipmentCategory[] = ['racket', 'string'];
...
export interface UseGearPicksOptions {
  /**
   ...
   * the same time.
   *
   * A callback rather than a second fetch in the card: this hook already owns
   ...
   *  throttled 200 renders as an error card. Format/budget changes (made from
   *  the pick sheet) are unaffected. */
  holdFitRefetch?: boolean;
}

export interface UseGearPicks {
  /** Per-category pick with reason KEYS already translated for this locale. */
  view: Record<EquipmentCategory, CategoryState>;
  /** A 403 from `/api/recommend`: this device does not own the name. */
  refused: boolean;
  /** Why each parked category parked — see `parkReasonRef` below. */
  parkReasons: Partial<Record<EquipmentCategory, ParkReason | null>>;
  /** A card's Try again. Re-reads the gear doc too when that is what failed. */
  retry: (cat: EquipmentCategory, status: GearPickCardStatus) => void;
...
/**
 * The `/api/recommend` reader for the Gear register: one fetch per SOURCED
 * category, the park/ready/error ladder, the refetch keys and their skip
 * rules, and the translation of reason keys at render.
 *
 * Extracted verbatim from `GearPickRail` (2026-09-14) so the Set-up card
 * (`NEXT_PUBLIC_FLAG_GEAR_SETUP`) can show the same picks without a second
 * reader. It is the same single-owner rule as `useGear`: exactly ONE instance
 * per register, owned by whichever component renders the register — the rail
 * on the flag-off branch, `GearRegister` on the flag-on one. Every comment
 * below was written about the rail and still holds word for word.
 */
export function useGearPicks(
  activeName: string | null,
  gear: UseGear,
  { onPairTension, holdFitRefetch = false }: UseGearPicksOptions = {},
): UseGearPicks {
  ...
  const feelKey = inPlay && !inPlay.catalogId && inPlay.feel
    ? `${inPlay.id}:${inPlay.feel.balance ?? ''}:${inPlay.feel.flex ?? ''}:${inPlay.feel.weight ?? ''}`
    : '';
