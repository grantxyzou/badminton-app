...
export const FLAGS: Record<FlagName, FlagMeta> = {
  ...
  NEXT_PUBLIC_FLAG_VALUE_HUB_SLICE: {
    ...
    owner: 'grant',
    plannedRemoval: 'after Slice-0 kill-criterion gate decides — either promoted to stable + lived-in 2 weeks, or torn down and the flag deleted along with the player-facing UI. NOTE: the 4-week clock RESTARTED 2026-08-16. The Equipment register had been parked under the assessment spine since v1.7, so the rec card never rendered on either deployment and the criterion was measuring an invisible surface. Read the gate with GET /api/admin/slice0 (admin-only) rather than eyeballing it.',
  },
  NEXT_PUBLIC_FLAG_NAV_RAIL: {
