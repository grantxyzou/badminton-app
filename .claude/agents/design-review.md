...
- Configure initial viewport (375x812 for mobile-first — this app is mobile-primary)
...
- Test both light and dark themes via the theme toggle

## Phase 2: Responsiveness Testing
- Test mobile viewport (375px) FIRST — this app is mobile-primary
- Test tablet viewport (768px) — verify layout adaptation
- Test desktop viewport (1440px) — capture screenshot
- Verify no horizontal scrolling or element overlap
- Verify thumb-reach zones for one-handed use on mobile

## Phase 3: Visual Polish
- Assess layout alignment and spacing consistency
- Verify typography hierarchy and legibility
- Check color palette consistency in BOTH themes
- Ensure visual hierarchy guides user attention
- Verify glass design effects (backdrop-filter, borders) render correctly in both themes
...
- Test color contrast ratios (4.5:1 minimum) in BOTH light and dark themes
- Verify touch target sizes meet 44x44px minimum for mobile
...
- Use `?dev` URL parameter (DevPanel) to test UI states: cost visibility, payment reminder, signed-up status, player counts

## Phase 6: Code Health
- Verify component reuse over duplication
- Check for design token usage (CSS custom properties in `globals.css`, no magic numbers)
- Ensure adherence to established patterns (named classes, segment-control, etc.)
- Confirm no hard divider lines between list items (project convention)

## Phase 7: Content and Console
- Review grammar and clarity of all text
- Check browser console for errors/warnings
- Verify no `0` renders from `{value && ...}` patterns (common React JSX pitfall noted in CLAUDE.md)
